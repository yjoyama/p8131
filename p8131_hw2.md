...
I employed the logistic regression model to investigate the relationship
between the scholarship amount and enrollment rate.  
Model: $g(P(enrolls))=\beta_0+\beta_1 X$

By the rule of thumb, there are not sufficient people in each
scholarship group. Therefore, I will conduct Hosmer-Lemeshow test to
...
The model assumes that when the amount scholarship is zero, the log odds
for enrollment among offer is -1.598. One unit increase of scholarship
amount (in thousand dollars) will increase the log odds of enrollment by
0.0163. With 95% confidence, the true $\beta_1$ lies within
(0.001-0.032).
...
We want to estimate $x_0$ such that
$\beta_0+\beta_1 x_0=g(p = 0.4)=log(\frac{0.4}{1-0.4})$
...
Therefore, we should provide 73,185 USD (95% CI: 45,516-100,855) to get
40% yield rate.
